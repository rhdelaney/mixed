...
Lexeme *evalGREAT_EQ(Lexeme *, Lexeme *);
Lexeme *evalGreater_than(Lexeme *, Lexeme *);
Lexeme *evalLESS_EQ(Lexeme *, Lexeme *);
...
Lexeme *evalFuncArgs(Lexeme *tree, Lexeme *params, Lexeme *env) {
  if((tree == NULL && params == NULL) || areEqual(tree,params,NIL)) {
    ...
    return lexeme(NIL);
  } else if(!strcmp(car(params)->type,OBRACE)) {
    //printf("thunk!\n");
...
Lexeme *stripOparen(Lexeme *tree) {
  if(tree == NULL || !strcmp(tree->type,NIL)) {
    ...
    if(!strcmp(car(tree)->type,OBRACE)) {
...
Lexeme *evalAssign(Lexeme *tree, Lexeme *env) {
  ...
  } else if(!strcmp((car(tree))->type,PERIOD)) {
...
Lexeme *evalGREAT_EQ(Lexeme *tree, Lexeme *env) {
  Lexeme *left = eval(car(tree),env);
  Lexeme *right = eval(cdr(tree),env);
  Lexeme *result = lexeme(INTEGER);
  result->ival = 0;
...
Lexeme *evalLESS_EQ(Lexeme *tree, Lexeme *env) {
  Lexeme *left = eval(car(tree),env);
  Lexeme *right = eval(cdr(tree),env);
  Lexeme *result = lexeme(INTEGER);
  result->ival = 0;
...
evalFunction getEvalFunction(char *type) {
  ...
  } else if(strcmp(type,GREAT_EQ) == 0) {
    return evalGREAT_EQ;
