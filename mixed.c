...
int main(int argc, char **argv) {
  ...
  FILE *stdlib = fopen("stdlib/stdlib.mx","r");
  ...
  //printf("before parsing std\n");
  //Lexeme *std=parse(stdlib);
 // printf("before eval?\n");
  eval(parse(stdlib),outer);
  //printf("before global out of eval?\n");
...
Lexeme *createBuiltin() {
  ...
  Lexeme *env = create();
 // printf("fucking created this shit fam\n");

  insert(print,defPrint,env);
  //printf("we inserted wtf?\n");
